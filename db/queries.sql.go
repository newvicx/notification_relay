...
func (q *Queries) DeleteCRAMCredential(ctx context.Context, username string) error {
	_, err := q.db.ExecContext(ctx, deleteCRAMCredential, username)
	return err
}
...
const getCRAMCredential = `-- name: GetCRAMCredential :one

SELECT username, secret_nonce, secret_cipher, roles, enabled, created_at FROM smtp_cram_credentials WHERE username = ? AND enabled = 1
`

// smtp_cram_credentials
func (q *Queries) GetCRAMCredential(ctx context.Context, username string) (SmtpCramCredential, error) {
	row := q.db.QueryRowContext(ctx, getCRAMCredential, username)
	var i SmtpCramCredential
	err := row.Scan(
		&i.Username,
		&i.SecretNonce,
		&i.SecretCipher,
...
const insertCRAMCredential = `-- name: InsertCRAMCredential :exec
INSERT INTO smtp_cram_credentials (username, secret_nonce, secret_cipher, roles, enabled, created_at)
VALUES (?, ?, ?, ?, ?, ?)
`

type InsertCRAMCredentialParams struct {
	Username     string `json:"username"`
...
func (q *Queries) InsertCRAMCredential(ctx context.Context, arg InsertCRAMCredentialParams) error {
	_, err := q.db.ExecContext(ctx, insertCRAMCredential,
		arg.Username,
		arg.SecretNonce,
		arg.SecretCipher,
		arg.Roles,
...
const listCRAMCredentials = `-- name: ListCRAMCredentials :many
SELECT username, secret_nonce, secret_cipher, roles, enabled, created_at FROM smtp_cram_credentials ORDER BY username
`

func (q *Queries) ListCRAMCredentials(ctx context.Context) ([]SmtpCramCredential, error) {
	rows, err := q.db.QueryContext(ctx, listCRAMCredentials)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []SmtpCramCredential
	for rows.Next() {
		var i SmtpCramCredential
		if err := rows.Scan(
			&i.Username,
