...
{{if .Done}}{{else}}
<form method="POST" action="{{.Prefix}}/subscribe">
...
<div class="footer">
  ...
  <a href="{{.Prefix}}/unsubscribe">Unsubscribe</a>
</div>
</body>
</html>
`))
...
{{if .Flash}}<div class="msg {{.FlashClass}}">{{.Flash}}</div>{{end}}
{{if .Done}}{{else}}
<form method="POST" action="{{.Prefix}}/unsubscribe">
  <label for="username">Username</label>
  <input type="text" id="username" name="username" autocomplete="username" required>
  <label for="password">Password</label>
  <input type="password" id="password" name="password" autocomplete="current-password" required>
...
{{end}}
<div class="footer">
  <a href="{{.Prefix}}/subscribe">Subscribe</a>
</div>
</body>
</html>
`))
...
type formPageData struct {
	...
	FlashClass string
	Done       bool
	// Prefix is the configured HTTP path prefix, set by renderPage; callers
	// building formPageData do not need to set it themselves.
	Prefix string
}

func (s *Server) handleSubscribeForm(w http.ResponseWriter, r *http.Request) {
	s.renderPage(w, subscribeFormTmpl, formPageData{})
}

func (s *Server) handleSubscribeSubmit(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil {
		s.renderPage(w, subscribeFormTmpl, formPageData{Flash: "Invalid form submission.", FlashClass: "error"})
		return
	}
	...
	if username == "" || password == "" {
		s.renderPage(w, subscribeFormTmpl, formPageData{Flash: "Username and password are required.", FlashClass: "error"})
		return
	}
	...
	if _, err := s.auth.AuthenticateUser(r.Context(), username, password); err != nil {
		...
		if errors.Is(err, ldap.ErrInvalidCredentials) {
			s.renderPage(w, subscribeFormTmpl, formPageData{Flash: "Invalid credentials.", FlashClass: "error"})
			return
		}
		s.logger.Error("subscribe: ldap auth error", "username", username, "error", err)
		s.renderPage(w, subscribeFormTmpl, formPageData{Flash: "Authentication error. Please try again later.", FlashClass: "error"})
		return
	}
	...
	if err != nil {
		...
		s.renderPage(w, subscribeFormTmpl, formPageData{Flash: "Could not retrieve account details. Please try again later.", FlashClass: "error"})
		return
	}
	...
	if err := s.q.InsertSMSSubscription(r.Context(), db.InsertSMSSubscriptionParams{
		...
		s.renderPage(w, subscribeFormTmpl, formPageData{Flash: "Registration failed. Please try again later.", FlashClass: "error"})
		return
	}
...
func (s *Server) handleUnsubscribeForm(w http.ResponseWriter, r *http.Request) {
	s.renderPage(w, unsubscribeFormTmpl, formPageData{})
}

func (s *Server) handleUnsubscribeSubmit(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil {
		s.renderPage(w, unsubscribeFormTmpl, formPageData{Flash: "Invalid form submission.", FlashClass: "error"})
		return
	}
	username := r.FormValue("username")
	password := r.FormValue("password")
	...
	if username == "" || password == "" {
		s.renderPage(w, unsubscribeFormTmpl, formPageData{Flash: "Username and password are required.", FlashClass: "error"})
		return
	}
	...
	if _, err := s.auth.AuthenticateUser(r.Context(), username, password); err != nil {
		...
		if errors.Is(err, ldap.ErrInvalidCredentials) {
			s.renderPage(w, unsubscribeFormTmpl, formPageData{Flash: "Invalid credentials.", FlashClass: "error"})
			return
		}
		s.logger.Error("unsubscribe: ldap auth error", "username", username, "error", err)
		s.renderPage(w, unsubscribeFormTmpl, formPageData{Flash: "Authentication error. Please try again later.", FlashClass: "error"})
		return
	}
	...
	if err := s.q.DeleteSMSSubscription(r.Context(), username); err != nil {
		...
		s.renderPage(w, unsubscribeFormTmpl, formPageData{Flash: "Unsubscribe failed. Please try again later.", FlashClass: "error"})
		return
	}
...
func (s *Server) renderPage(w http.ResponseWriter, tmpl *template.Template, data formPageData) {
